 ...
 *
 * @param lazy if true, the mapper will only map elements when they are requested otherwise it will
 *   build a cache upon creation
 */
class DAMapper
  @JvmOverloads
  constructor(
    ...
    fun getMappings(elementId: String): Set<DAMapping> {
      return mappings
        .filter { it.dstElementId == elementId || it.architectureElementId == elementId }
        .toSet()
    }
    ...
    fun mapToArchitecturalEntity(dstElementId: String): ArchitectureEntity {
      return mappingCache.computeIfAbsent(dstElementId) {
        val mapping =
          mappings.find { it.dstElementId == dstElementId }
    ...
    fun mapToDSTEntity(architectureElementId: String): DSTElement {
      return backMappingCache.computeIfAbsent(architectureElementId) {
        val mapping =
          mappings.find { it.architectureElementId == architectureElementId }
            ?: throw NoSuchElementException(
              "No mapping for architecture element $architectureElementId found.",
            )
